###### Code for the RMST presentation
...
surv <- Surv(time=data$surv.time, event=data$event==1)
surv1 <- Surv(time=trt1$surv.time, event=trt1$event==1)
surv0 <- Surv(time=trt0$surv.time, event=trt0$event==1)

kmfit <- survfit(surv~1)

plot(kmfit, col="blue", xlab = "Survival Time In Months", ylab = "Survival Probability", 
     conf.int=FALSE)

uplim <- 36
ylim <- kmfit$surv[kmfit$time<=uplim]

polygon(x=c(0,uplim), y=ylim, col="green")

abline(v=36, col="red")

expon <- curve(exp(-(log(2)/12)*x), from=0,to=60)

curve(exp(-(log(2)/12)*x), from=0,to=60, col="blue", xlab = "Survival Time In Months", 
      ylab = "Survival Probability")
abline(v=24, col="red")
polygon(c(0,x[x<=24]),c(y[x<=24],0),col="green")
...
y <- exp(-(log(2)/12)*x)
plot(x,y,type="l", col="blue", xlab = "Survival Time In Months", 
     ylab = "Survival Probability", main="Mean Survival")
polygon(c(x,max(x),0),c(y,0,0),col="green")


data("pbc")
pbc = na.omit(pbc)
for (i in 1:nrow(pbc)){
     pbc$status[i] <- max(0,pbc$status[i]-1)
     if (pbc$sex[i]=="f"){pbc$fem[i]<-1}else{pbc$fem[i]<-0}
}
pbc$trt <- pbc$trt-1
sink(file="output1.txt")
RMSTdiff(data=pbc,
...
Y <- Surv(pbc$time, data$status)

kmfit <- survfit(Y~data$trt)
